...
<script>
  ...
  function getAllCompanies(type = null) {
    ...
    // sent ajax request
    jQuery.ajax({
      url: "http://hypertester.ir/serverHypernetShowUnion/showCompanies.php",
      method: "POST",
      data: JSON.stringify(data),
      dataType: "json",
      contentType: "application/json",
      success: function(data) {
        ...
        data.forEach(function(item, index) {
          ...
          <ul class="nav justify-content-end companyActions" style="display:flex">
            ...
            <li class="nav-item">
              <a class="nav-link btn companyProductAction${item['user_id']}" ${item['user_id']==null ? 'disabled' : ''} href="#" ${item['user_id'] ? `onclick="getAllProductCompany(event,${item['id']})"` : 'onclick="function(event){event.preventDefault()}"' } > محصولات</a>
            </li>
          </ul>
          </div>`;
    ...
    let modal = `
      <div class="row">
        <div id="sp-component" class="col-sm-12 col-md-12">
          <div class="sp-column ">
            <div id="system-message-container" id="statusTextUpdateBrand"></div>
              <form method="post" id="userForm" onsubmit="updateBrand(event,this)" class="nextForm formResponsive" action="http://hypertester.ir/index.php?option=com_rsform&amp;view=rsform&amp;formId=62">
